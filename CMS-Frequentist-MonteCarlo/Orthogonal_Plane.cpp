...
#include <cmath>
#include <limits> 
Orthogonal_Plane::Orthogonal_Plane(std::vector<std::vector<double>> points)
{
	pointSet = points;
...
bool Orthogonal_Plane::Check_Plane()
{
	...
	std::vector<double> m(nDims-1, 0);
	std::vector<double> v(nDims-1, 0);
	double oldLoss;
...
std::vector<double> Orthogonal_Plane::grad()
{
	std::vector<double> gradVect(nDims-1);
...
void Orthogonal_Plane::findStartPoint()
{
	...
	{
		std::vector<double> testPoint(nDims,epsilon);
		testPoint[i] = 1;
