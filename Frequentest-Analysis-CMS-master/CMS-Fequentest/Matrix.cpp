...
{
	typedef typename unsigned index_type;
...
template<>
struct matrix_traits<vector<vector<double, double>, double>>
{
	typedef std::size_t index_type;
	typedef double value_type;
	static index_type min_row(const vector<vector<double>>&)
	{
		return 0;
	}
	static index_type max_row(const vector<vector<double>>& vec)
	{
		return vec.size() - 1;
	}
	static index_type min_column(const vector<vector<double>>&)
	{
		return 0;
	}
	static index_type max_column(const vector<vector<double>>& vec)
	{
		return vec[0].size() - 1;
	}
	static value_type& element(vector<vector<double>>& A,
		index_type i, index_type k)
	{
		return A[i][k];
	}
	static value_type element(const vector<vector<double>> &A,
		index_type i, index_type k)
	{
		return A[i][k];
	}
